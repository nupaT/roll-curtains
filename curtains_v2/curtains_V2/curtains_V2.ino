...
void loop() 
{
 ...
 if(butt.isClick()) {         //если кнопка нажата - меняем статус мотора на ON (двигается)
  ...
  if(moveStatus1 /*&& setStatus1 == 1*/) {    //если мотор ON(двигается) и реверс мотора выключен
    moveMotor(distance1);     //запускаем мотор от 0 до distance1 (прямое направление)
  } /*else if (moveStatus1 && setStatus1 == -1) {   //иначе если мотор ON и статус реверса ВКЛ
...
void moveMotor(long dist){
    motor1.setTarget(dist*setStatus1);  //устанавливаем конечное положение для мотора ()
...
//      Serial.println("FINISHED!");
      ...
      setStatus1 = (setStatus1*(-1));   //меняем значение реверса на противоположное
